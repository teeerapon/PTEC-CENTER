...
export default function FormsStart() {
  ...
  const handleUploadFile_2 = async (e, index) => {
    ...
    if (['jpg', 'png', 'gif', 'xbm', 'tif', 'pjp', 'svgz', 'jpeg', 'jfif', 'bmp', 'webp', 'svg'].indexOf((e.target.files[0].name).split('.').pop()) > -1) {
      ...
      await Axios.post(config.http + "/check_files_NewNAC", formData_1, { headers })
        .then(async (res) => {
          const list = [...serviceList];
          list[index]['image_2'] = 'http://vpnptec.dyndns.org:33080/NEW_NAC/' + res.data.attach[0].ATT + '.' + e.target.files[0].name.split('.').pop();
          setTooltipImage_2(e.target.files[0].name)
          setServiceList(list)
          const req = {
            usercode: data.UserCode,
            dtl_id: list[index].dtl_id,
                        ...
                        .then((resIII) => {
                          if (i + 1 === serviceList.length) {
                            swal("แจ้งเตือน", 'อัปเดตรายการแล้ว', "success", { buttons: false, timer: 2000 }).then((value) => {
                              ...
                              window.location.href = '/NAC/AddAssets_2?' + pathLink
                            });
                          }
                        })
                    }
  ...
  const handleSubmit_Form = async () => {
    if ((sendHeader[0].nac_status === 4 || sendHeader[0].nac_status === 14)
      && serviceList.filter((res) => res.statusCheck === 0 || !res.image_1 || !res.image_2)[0]) {
      swal("แจ้งเตือน", `เลือก (ตรวจสอบ/รูปภาพ) ทรัพย์สิน`, "error")
    } else if (!sendHeader[0].des_delivery || (sendHeader[0].nac_status === 4 && !desName) || (sendHeader[0].nac_status === 4 && !desLastName)) {
  ...
  } else if (sendHeader[0].nac_type === '1' || sendHeader[0].nac_type === 1) {
    return (
      <React.Fragment>
        <CssBaseline />
        <AppbarNAC
                      ...
                      >
                        <Grid item xs={6}>
                          ...
                          <TextField
                            disabled
                            inputProps={{ style: textAlignDisabled }}
                            value={sendHeader[0].des_department}
                            name='des_Dep'
                            variant="standard"
                          />
                        </Grid>
                        ...
                        <Grid item xs={6}>
                          <TextField
                            ...
                            onChange={(e) => {
                              const list = [...sendHeader]
                              list[0].nameDes = `${e.target.value} ${desLastName ? desLastName : ''}`
                        ...
                        <Grid item xs={6}>
                          <TextField
                            ...
                            onChange={(e) => {
                              const list = [...sendHeader]
                              list[0].nameDes = `${desName ? desName : ''} ${e.target.value}`
                        ...
                        <Grid item xs={12}>
                          <LocalizationProvider dateAdapter={AdapterDayjs}>
                            <DateTimePicker
                              ...
                              sx={{ width: '100%' }}
                              timezone="UTC"
                              value={sendHeader[0].des_deliveryDate ? dayjs(sendHeader[0].des_deliveryDate) : null}
                              onChange={(newValue) => {
                                const listHeader = [...sendHeader]
                                listHeader[0]['des_deliveryDate'] = dayjs(newValue).format('YYYY-MM-DD HH:mm:ss')
                                setSendHeader(listHeader)
                              }}
                              ampm={false}
                              label='วันที่ส่งมอบ'
                                  ...
                                  <Stack direction="row" spacing={1}>
                                    ...
                                    <Tooltip title='delete image 1'>
                                      <IconButton component="label">
                                        <ClearIcon className='scaled-icon-table' onClick={(e) => handleCancelUploadFile_1(e, index)} />
                                      </IconButton>
                                    </Tooltip>
                                  </Stack>
                                </React.Fragment>
                              }
                              {res.image_2 === '' || !res.image_2 ?
                                <React.Fragment>
                                ...
                                <React.Fragment>
                                  <Stack direction="row" spacing={1}>
                                    ...
                                    <Tooltip title='delete image 2'>
                                      <IconButton component="label">
                                        <ClearIcon className='scaled-icon-table' onClick={(e) => handleCancelUploadFile_2(e, index)} />
                                      </IconButton>
                                    </Tooltip>
                                  </Stack>
                                </React.Fragment>
