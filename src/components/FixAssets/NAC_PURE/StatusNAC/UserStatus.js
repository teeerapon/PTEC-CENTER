...
export default function History_of_assets() {
  ...
  const change_page_NacOperation = (newPage) => {
    localStorage.setItem('pagination_user', JSON.stringify(newPage));
            ...
            <Grid container rowSpacing={1}>
              ...
              <Grid item xs={6}>
                <Button
                  ...
                  color="error"
                  disabled={
                    (checkUserWeb === 'admin' || data.UserCode === params.row.create_by || data.UserCode === params.row.source_userid)
                      && ((params.row.nac_status < 4 && params.row.nac_status !== 11) && (params.row.nac_status < 4 && params.row.nac_status !== 6)) ? false : true
                  }
